...
所以，我们在调用 `find_cached_set` 前对目标 `dependency` 添加 `external source` ，且使用 `AOP` 编程以兼容 `CocoaPods` 升级：
...
在组件化架构向 `monorepo` 方案演进过程中， 我们发现 `CocoaPods` 本身无法很好地满足要求。但幸运的是，插件机制为支持 `monorepo` 特性提供了可能。同时在 `monorepo` 插件正式交付后，很好地支撑了公司多条产品线进行，实现了最大化收益。
